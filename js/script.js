...
var actualFat = 0;
var actualProtein = 0;
//Object array to store meal inputs (for total calorie intake query)
var mealObject = {
    mealOne: {
        oneItems: [],
        oneWeights: [],
...
//Form submit to handle about inputs. Captures values and checks if they are within require parameters, progress to recommended calories modal if successful.
$("#about-form").on("submit", function (event) {
    ...
    if(age>80 || age<0){
        warningMessage = "Please ensure age is between 0 and 80."
        $("#about-warning").remove()
        $("#about-form").append(createWarningElement(warningMessage))
        return;
    }
    ...
    if(height<130 || height>230){
        warningMessage = "Please ensure height is between 130 and 230."
        $("#about-warning").remove()
        $("#about-form").append(createWarningElement(warningMessage))
        return;
    }
    ...
    if(weight<40 || weight>160){
        warningMessage = "Please ensure weight is between 40 and 160."
        $("#about-warning").remove()
        $("#about-form").append(createWarningElement(warningMessage))
        return;
    }
